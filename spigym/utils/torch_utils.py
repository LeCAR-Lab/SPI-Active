...
@torch.jit.script
def transform_by_quat(pos,quat):
    # Unbind the quaternion in xyzw convention (x, y, z, w)
    qx, qy, qz, qw = quat.unbind(-1)
    batch_dims = quat.shape[:-1]

    # Construct the rotation matrix using the components in the xyzw convention
...
def torch_rand_float_normal(mean, std, shape, device):
    return mean + std * torch.clamp(torch.randn(*shape, device=device),-1,1)
