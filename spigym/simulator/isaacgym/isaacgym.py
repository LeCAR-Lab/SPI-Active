...
import torch
from spigym.utils.torch_utils import to_torch, torch_rand_float
import numpy as np
from termcolor import colored
from collections import deque
import cv2
from datetime import datetime
from spigym.envs.env_utils.terrain import Terrain
from rich.progress import Progress
from spigym.simulator.base_simulator.base_simulator import BaseSimulator
...
class IsaacGym(BaseSimulator):
    ...
    def _parse_sim_params(self):
        ...
        # Ensure sufficient PhysX GPU memory for large environment batches
        physx = sim_params.physx
        target_capacity = 80_000_000
        if hasattr(physx, "contact_pairs_capacity"):
            physx.contact_pairs_capacity = max(physx.contact_pairs_capacity, target_capacity)
        if hasattr(physx, "max_gpu_contact_pairs"):
            physx.max_gpu_contact_pairs = max(physx.max_gpu_contact_pairs, target_capacity)
        if hasattr(physx, "max_gpu_shape_pairs"):
            physx.max_gpu_shape_pairs = max(physx.max_gpu_shape_pairs, target_capacity)
        if hasattr(physx, "found_lost_pairs_capacity"):
            physx.found_lost_pairs_capacity = max(physx.found_lost_pairs_capacity, target_capacity)
        if hasattr(physx, "found_lost_aggregate_pairs_capacity"):
            physx.found_lost_aggregate_pairs_capacity = max(
                physx.found_lost_aggregate_pairs_capacity, target_capacity
            )
        if hasattr(physx, "default_buffer_size_multiplier"):
            physx.default_buffer_size_multiplier = max(
                physx.default_buffer_size_multiplier, 50
            )
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.env_config.domain_rand.randomize_base_inertia:
            self._base_inertia_bias = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
            if env_id<3:
                logger.debug("randomizing base inertia")
            # try:
            #     torso_index = self._body_list.index("torso_link")
            # except:
            #     torso_index = self._body_list.index("pelvis") # for fixed upper URDF we only have pelvis link
            # assert torso_index != -1
            torso_index = 0
            inertia_x_bias = np.random.uniform(self.env_config.domain_rand.base_inertia_range.x[0], self.env_config.domain_rand.base_inertia_range.x[1])
            inertia_y_bias = np.random.uniform(self.env_config.domain_rand.base_inertia_range.y[0], self.env_config.domain_rand.base_inertia_range.y[1])
            inertia_z_bias = np.random.uniform(self.env_config.domain_rand.base_inertia_range.z[0], self.env_config.domain_rand.base_inertia_range.z[1])

            self._base_inertia_bias[env_id, 0] += inertia_x_bias
            self._base_inertia_bias[env_id, 1] += inertia_y_bias
            self._base_inertia_bias[env_id, 2] += inertia_z_bias

            props[torso_index].inertia.x.x += inertia_x_bias
            props[torso_index].inertia.y.y += inertia_y_bias
            props[torso_index].inertia.z.z += inertia_z_bias
    ...
    def setup_viewer(self):
        ...
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_L, "increase_phase_command"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_K, "decrease_phase_command"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_J, "increase_offset_command"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_H, "decrease_offset_command"
        )

        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_G, "increase_bound_command"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_F, "decrease_bound_command"
        )

        sim_params = self.sim_params
        if sim_params.up_axis == gymapi.UP_AXIS_Z:
            cam_pos = gymapi.Vec3(0.35, -2.5, 0.5)
    ...
    def render(self, sync_frame_time=True):
        ...
        for evt in self.gym.query_viewer_action_events(self.viewer):
            ...
            #     self.next_task()
                ...
                self.apply_force_tensor[:, self.right_hand_link_index, 2] -= 1.0
                logger.info(f"Right hand force: {self.apply_force_tensor[:, self.right_hand_link_index, :]}")
            elif evt.action == "increase_phase_command" and evt.value > 0:
                self.commands[:, 5] = max(min(self.commands[:, 5] + 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 5]}")
            elif evt.action == "decrease_phase_command" and evt.value > 0:    
                self.commands[:, 5] = max(min(self.commands[:, 5] - 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 5]}")
            elif evt.action == "increase_offset_command" and evt.value > 0:
                self.commands[:, 6] = max(min(self.commands[:, 6] + 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 6]}")
            elif evt.action == "decrease_offset_command" and evt.value > 0:    
                self.commands[:, 6] = max(min(self.commands[:, 6] - 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 6]}")
            elif evt.action == "increase_bound_command" and evt.value > 0:
                self.commands[:, 7] = max(min(self.commands[:, 7] + 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 7]}")
            elif evt.action == "decrease_bound_command" and evt.value > 0:    
                self.commands[:, 7] = max(min(self.commands[:, 7] - 0.1, 1.0), 0.0)
                logger.info(f"Current Command: {self.commands[:, 7]}")
