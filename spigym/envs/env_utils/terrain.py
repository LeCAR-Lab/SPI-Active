...
class Terrain:
    ...
    def block_terrain_func(self, terrain, difficulty):
        block_height = self.cfg.block_height
        block_width = self.cfg.block_width
        block_width = int(block_width / terrain.horizontal_scale)
        

        terrain.height_field_raw[:, :] = 0.0
        terrain.height_field_raw[:, -block_width:] = int(block_height/terrain.vertical_scale)
