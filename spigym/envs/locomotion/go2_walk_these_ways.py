...
class go2_tracking_interface(LeggedRobotLocomotion):
    ...
    def _compute_reward(self):
        ...
        elif self.config.rewards.wtw_style:
    ...
    def _init_command_distribution(self,env_ids):
        ...
        for category in self.category_names:
            self.curricula+= [CurriculumClass(seed=self.config.obs.commands.curriculum_seed,
                                               ...
                                               aux_reward_coef=(self.config.obs.commands.limit_aux_reward_coef[0],
                                                           self.config.obs.commands.limit_aux_reward_coef[1],
                                                           self.config.obs.commands.num_bins_aux_reward_coef),
                                               )]
        ...
        low = np.array(
            [self.config.obs.commands.lin_vel_x[0], self.config.obs.commands.lin_vel_y[0],
             ...
             self.config.obs.commands.stance_length_range[0], self.config.obs.commands.aux_reward_coef_range[0], ])
        high = np.array(
            [self.config.obs.commands.lin_vel_x[1], self.config.obs.commands.lin_vel_y[1],
             ...
             self.config.obs.commands.stance_length_range[1], self.config.obs.commands.aux_reward_coef_range[1], ])
        for curriculum in self.curricula:
    ...
    def step(self, actions):
        if EVAL_after_real:
            ...
            self.log_states({
                    ...
                    "desired_contact_states": self.desired_contact_states[0].cpu().numpy()
                })
            self.vel_cmd_coutner += 1
