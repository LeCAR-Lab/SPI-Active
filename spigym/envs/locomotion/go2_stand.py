...
class go2_stand(LeggedRobotLocomotion):
    ...
    def _reward_orientation_control(self):
        ...
        if self.config.rewards.task == "stand":
            quat_pitch = quat_from_angle_axis(1*torch.pi*phase, torch.tensor([0., 1., 0.], device=self.device))
        else:
            quat_pitch = quat_from_angle_axis(-1*torch.pi*phase, torch.tensor([0., 1., 0.], device=self.device))
    ...
    def _reward_roll_orientation_control(self):
        current_time  = self.episode_length_buf*self.dt
        phase = (current_time - 0.5).clamp(min=0, max=0.5)
        quat_roll = quat_from_angle_axis(0*phase, torch.tensor([1.0, 0., 0.], device=self.device))
    ...
    def _reward_ang_vel_y(self):
        current_time = self.episode_length_buf * self.dt
        ang_vel = -self.base_ang_vel[:, 1].clamp(max=4.2, min=-4.2)
        if self.config.rewards.task == "stand":
    ...
    def _reward_lin_vel_z(self):
        current_time = self.episode_length_buf * self.dt
        lin_vel = self.base_lin_vel[:, 2].clamp(max=2.0)
        return lin_vel * torch.logical_and(current_time > 0.5, current_time < 0.65) #- lin_vel * torch.logical_and(current_time > 1.5, current_time < 2.0)
    ...
    def _reward_height_control(self):
        ...
        current_time = self.episode_length_buf * self.dt
        target_height = self.config.rewards.base_height_target
        ...
        return torch.exp(-height_diff/self.config.rewards.reward_tracking_sigma.height) 
    ...
    def _reward_penalty_forward_feet_contact(self):
        if self.config.rewards.task == "stand":
            front_feet_indices = self.feet_indices[:2]
        else:
            front_feet_indices = self.feet_indices[2:]
        return torch.sum(torch.norm(self.simulator.contact_forces[:, front_feet_indices, :], dim=-1) > 1., dim=1)
    
    def _reward_rear_feet_contact(self):
        if self.config.rewards.task == "stand":
            rear_feet_indices = self.feet_indices[2:]
        else:
            rear_feet_indices = self.feet_indices[:2]
        feet_forces = self.simulator.contact_forces[:, rear_feet_indices, :]
        feet_contact = torch.norm(feet_forces, dim=2)
        all_feet_on_ground = torch.all(feet_contact >0, dim=1)        
        return (all_feet_on_ground)*1.0
    ...
    def _reward_penalty_slippage(self):
        ...
        return torch.sum(torch.norm(foot_vel, dim=-1) * (torch.norm(self.simulator.contact_forces[:, self.feet_indices, :], dim=-1) > 1.), dim=1)*torch.logical_or(current_time < 0.5, current_time > 1.0)
    ...
    def _reward_feet_height_before_stand(self):
        current_time = self.episode_length_buf * self.dt
        feet_height = self.simulator._rigid_body_pos[:,self.feet_indices, 2].view(self.num_envs, -1) - 0.02
        return torch.exp(-feet_height.clamp(min=0).sum(dim=1)/self.config.rewards.reward_tracking_sigma.height) * (current_time < 0.5)
